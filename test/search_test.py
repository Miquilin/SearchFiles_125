...
class SearchTest(unittest.TestCase):
   ...
   def test_search_by_exact_common_name_with_extension_that_returns_coincidences(self):
        ...
        for item in search_test:
            if isinstance(item, File):
                for file in files:
                    file_namea , separatora, extensiona = file.partition(".")

                    if extensiona != "":
                        self.assertTrue(file_namea == item.get_file_name())
                        self.assertTrue(extensiona == item.get_extension())

                    elif file_namea == item.get_file_name():
                        self.assertTrue(file_namea == item.get_file_name())
            elif isinstance(item, Directory):
                for dir in directories:
                    if dir in item.get_dir_name():
   ...
   def test_search_by_exact_common_extension_that_returns_coincidences(self):
        ...
        for item in search_test:
            if isinstance(item, File):
                for file in files:
                    file_namea , separatora, extensiona = file.partition(".")
                    if file_namea == item.get_file_name():
                        self.assertTrue(extensiona == item.get_extension())
